...
function GetRS5kID(tr)
    ...
    for i = 1, reaper.TrackFX_GetCount(tr) do
        ...
        -- Check for either "RS5K" or "ReaSamplOmatic5000" in the FX name
        if fx_name:find('RS5K') or fx_name:find('ReaSamplOmatic5000') then
            return i - 1 -- Return the index of the RS5K instance
        end
    end
    return id -- Return -1 if no RS5K or ReaSamplomatic is found
end
...
-------------------------------------------------------------------------------
function ClearExistingSamples(track, rs5k_pos)
    -- Check if a valid ReaSamplomatic instance exists
    if rs5k_pos == -1 then
        reaper.ShowMessageBox("No ReaSamplomatic found on the selected track!", "Error", 0)
        return
    end
    
    -- Clear all sample slots (FILE0 to FILE127) in the ReaSamplomatic instance
    for i = 0, 127 do
        reaper.TrackFX_SetNamedConfigParm(track, rs5k_pos, "FILE" .. i, "")
    end
    
    --reaper.ShowMessageBox("Cleared all samples in ReaSamplomatic!", "Success", 0)
end
------------------------------------------------------------------------------- 
