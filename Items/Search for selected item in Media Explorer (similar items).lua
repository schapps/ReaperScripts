-- @description Search for selected item in Media Explorer (similar items)
-- @author Stephen Schappler
-- @version 1.0
-- @about
--   Copies the file name of the selected item to the clipboard. 
--   Sanitizes the name by removing suffixes after _PSE and replacing numbers with spaces.
...
if item then
    ...
    if take and not reaper.TakeIsMIDI(take) then
        ...
        if file_path and file_path ~= "" then
            ...
            if file_name then
                local sanitized_name = file_name

                -- Keep only the portion up to and including "_PSE" if present
                local pse_index = sanitized_name:find("_PSE")
                if pse_index then
                    sanitized_name = sanitized_name:sub(1, pse_index + 3)
                end

                -- Replace any sequence of digits with a single space
                sanitized_name = sanitized_name:gsub("%d+", " ")

                -- Collapse consecutive spaces and trim leading/trailing whitespace
                sanitized_name = sanitized_name:gsub("%s+", " ")
                sanitized_name = sanitized_name:match("^%s*(.-)%s*$") or sanitized_name
